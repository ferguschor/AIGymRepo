...
from tensorflow.keras.layers import Input, Dense, Conv2D, ELU, BatchNormalization, Flatten, ReLU
...
class PGNetwork:
    ...
    def __init__(self, input_shape, num_actions,  lr, vf_coef, ent_coef):
        ...
        conv = Conv2D(filters=32, kernel_size=[8, 8], strides=[4, 4], kernel_initializer=tf.initializers.orthogonal(gain=np.sqrt(2)), name="Conv1")(input)
        # conv = BatchNormalization(epsilon=1e-5, name="Conv1_BN")(conv)
        conv = ReLU(name="Conv1_RELU")(conv)
        conv = Conv2D(filters=64, kernel_size=[4, 4], strides=[2, 2], kernel_initializer=tf.initializers.orthogonal(gain=np.sqrt(2)), name="Conv2")(conv)
        # conv = BatchNormalization(epsilon=1e-5, name="Conv2_BN")(conv)
        conv = ReLU(name="Conv2_RELU")(conv)
        conv = Conv2D(filters=64, kernel_size=[3, 3], strides=[1, 1], kernel_initializer=tf.initializers.orthogonal(gain=np.sqrt(2)), name="Conv3")(conv)
        # conv = BatchNormalization(epsilon=1e-5, name="Conv3_BN")(conv)
        conv = ReLU(name="Conv3_RELU")(conv)

        flatten = Flatten(name="Flatten")(conv)
        dense = Dense(512, activation="relu", name="Dense1")(flatten)
