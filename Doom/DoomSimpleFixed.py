...
def td_train(load_weights=False, timed=False):
    ...
    for episode in range(total_episodes):
        ...
        while step < max_steps:
            ...
            next_Q = np.max(DQN.model(next_state_batch), axis=1)
            ...
            print("NA Input: ", np.any(np.isnan(current_state_batch)))
            print("NA Target: ", np.any(np.isnan(target_Q_batch)))
...
def simple_overfit():
    ...
    img = tf.random.uniform((5,84,84,4))
