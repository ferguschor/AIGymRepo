...
import datetime
import time
from ConvPGNetwValue import ConvPGNet
...
def get_batch(action_policy, batch_size=500, verbose=False):
    ...
    while total_reward_count < batch_size:
        ...
        while not done:
            ...
            # Model output = action_prob, state_value
            model_output = action_policy.adv_model(tf.reshape(current_state, [-1, *state_size]))
            action_prob = np.array(model_output)
            # print(action_prob)
...
if __name__ == "__main__":
    ...
    train_log_dir = 'C:/Users/Fergus/PycharmProjects/AIGym/AIGymRepo/PolicyGradient/DoomHealth/logs/' + current_time + '/train'
    train_summary_writer = tf.summary.create_file_writer(train_log_dir)

    action_policy = ConvPGNet(state_size, num_actions=num_actions, adv_lr=learning_rate, value_lr=value_lr)
    ...
    for i_epoch in range(max_epochs):
        ...
        policy_loss, value_loss, entropy_loss, loss = action_policy.fit_gradient_w_value(states_mb, actions_mb, discounted_rewards_mb)
        print("===============================================")
        ...
        with train_summary_writer.as_default():
            ...
            tf.summary.scalar('PolicyLoss', policy_loss, step=i_epoch)
            tf.summary.scalar('ValueLoss', value_loss, step=i_epoch)
            tf.summary.scalar('EntropyLoss', entropy_loss, step=i_epoch)
            ...
            for i, layer in enumerate(action_policy.adv_model.layers):
                if isinstance(layer, (tf.keras.layers.Conv2D, tf.keras.layers.Dense)):
                    w = layer.get_weights()[0]
                    b = layer.get_weights()[1]
                    tf.summary.histogram("AdvLayer"+str(i)+"Weights", w, step=i_epoch)
                    tf.summary.histogram("AdvLayer"+str(i)+"Bias", b, step=i_epoch)
                    tf.summary.scalar("AdvLayer"+str(i)+"WeightsMean", np.mean(w), step=i_epoch)
                    tf.summary.scalar("AdvLayer"+str(i)+"BiasMean", np.mean(b), step=i_epoch)
                    tf.summary.scalar("AdvLayer"+str(i)+"WeightsStd", np.std(w), step=i_epoch)
                    tf.summary.scalar("AdvLayer"+str(i)+"BiasStd", np.std(b), step=i_epoch)
            for i, layer in enumerate(action_policy.value_model.layers):
                if isinstance(layer, (tf.keras.layers.Conv2D, tf.keras.layers.Dense)):
                    w = layer.get_weights()[0]
                    b = layer.get_weights()[1]
                    tf.summary.histogram("ValueLayer"+str(i)+"Weights", w, step=i_epoch)
                    tf.summary.histogram("ValueLayer"+str(i)+"Bias", b, step=i_epoch)
                    tf.summary.scalar("ValueLayer"+str(i)+"WeightsMean", np.mean(w), step=i_epoch)
                    tf.summary.scalar("ValueLayer"+str(i)+"BiasMean", np.mean(b), step=i_epoch)
                    tf.summary.scalar("ValueLayer"+str(i)+"WeightsStd", np.std(w), step=i_epoch)
                    tf.summary.scalar("ValueLayer"+str(i)+"BiasStd", np.std(b), step=i_epoch)
        
        if i_epoch % epoch_save == 0:
            action_policy.adv_model.save_weights('C:/Users/Fergus/PycharmProjects/AIGym/AIGymRepo/PolicyGradient/DoomHealth/checkpoint/adv_checkpoint')
            action_policy.value_model.save_weights('C:/Users/Fergus/PycharmProjects/AIGym/AIGymRepo/PolicyGradient/DoomHealth/checkpoint/value_checkpoint')
