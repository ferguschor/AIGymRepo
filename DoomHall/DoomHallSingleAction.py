...
def td_train(load_weights=False, timed=False, verbose=False):
    ...
    for episode in range(total_episodes):
        ...
        while step < max_steps:
            ...
            target_Q_batch = np.zeros(next_Q.shape, dtype=np.float32)
            ...
            for i in range(len(training_batch)):
                if done_batch[i]:
                    ...
                    target_Q_batch[i] = reward_batch[i] + discount_rate * next_Q[i]
            ...
            loss, abs_errors = DQN.single_action_gradient(current_state_batch, action_batch, target_Q_batch, ISWeights_batch)
...
def test_agent(n):
    ...
    for i in range(n):
        ...
        while not game.is_episode_finished() and steps < max_test_steps:
            ...
            output = DQN.model(np.reshape(current_state, [-1, *state_size]))
            print("\t",output)
            action_index = np.argmax(output, axis=1)[0]
            action = possible_actions[action_index]
